...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return (runningCaloriesMeanSpeedMultiplier * avgSpeed * runningCaloriesMeanSpeedShift) * (weight / mInKm) * duration * minInH
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	avgSpeed := meanSpeed(action, duration) * kmhInMsec
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(avgSpeed, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
